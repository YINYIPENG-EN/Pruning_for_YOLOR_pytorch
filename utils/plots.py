...
def plot_labels(labels, save_dir=''):  # # labels的形式：【类，box】
    """
    该函数会绘制4个子图：
    ...
    """
    # plot dataset labels
    # labels[:,0]:取标签，labels[:,1:]取boxes,labels[:, 1:].transpose()转置，实际b=boxes的转置
    c, b = labels[:, 0], labels[:, 1:].transpose()  # classes, boxes
    nc = int(c.max() + 1)  # number of classes

    # 2*2的子图【可以放4张图】，大小为8*8，fig是绘制的画布，ax是要绘制的对象子图
    fig, ax = plt.subplots(2, 2, figsize=(8, 8), tight_layout=True)
    ax = ax.ravel()
    # 用柱状图长度表示类出现的频率
    ax[0].hist(c, bins=np.linspace(0, nc, nc + 1) - 0.5, rwidth=0.8)
    # 设置横坐标label代表‘classes’
    ax[0].set_xlabel('classes')
    # 散点图，绘制boxes的中心点。b[0]是boxes的center_x,b[1]是所有box的center_y
    ax[1].scatter(b[0], b[1], c=hist2d(b[0], b[1], 90), cmap='jet')
    ax[1].set_xlabel('x')
    ax[1].set_ylabel('y')
    # 散点图：绘制boxes的w,h，横坐标为width,纵坐标为height
