...
def create_modules(module_defs, img_size, cfg):
    ...
    _ = module_defs.pop(0)  # cfg training hyperparams (unused) module_defs[0]是net内容，训练参数
    output_filters = [3]  # input channels
    module_list = nn.ModuleList()
    routs = []  # list of layers which rout to deeper layers
    yolo_index = -1

    for i, mdef in enumerate(module_defs):  # 读取CNN各个参数
        modules = nn.Sequential()

        if mdef['type'] == 'convolutional':  # 获得卷积层
            bn = mdef['batch_normalize']  # bn层
            filters = mdef['filters']  # 获得输出通道
            k = mdef['size']  # kernel size  获得卷积核size
            stride = mdef['stride'] if 'stride' in mdef else (mdef['stride_y'], mdef['stride_x'])  # 获得步长
            if isinstance(k, int):  # single-size conv  放入卷积层
        ...
        elif mdef['type'] == 'deformableconvolutional': #变形卷积
        ...
        elif mdef['type'] == 'dropout':  # 采用dropout
            p = mdef['probability']
            modules = nn.Dropout(p)

        elif mdef['type'] == 'avgpool':  # 平均池化
            modules = GAP()
        ...
        elif mdef['type'] == 'shift_channels':  # nn.Sequential() placeholder for 'shortcut' layer  转化通道
            layers = mdef['from']
            filters = output_filters[-1]
            routs.extend([i + l if l < 0 else l for l in layers])
            ...
            modules = YOLOLayer(anchors=mdef['anchors'][mdef['mask']],  # anchor list
                                nc=mdef['classes'],  # number of classes  获取类的数量
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.module_defs = parse_model_cfg(cfg)  # 返回cfg列表  获取网络模型
    ...
    def forward(self, x, augment=False, verbose=False):  # x是数据集图像
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            if name in ['WeightedFeatureFusion', 'FeatureConcat', 'FeatureConcat2', 'FeatureConcat3', 'FeatureConcat_l', 'ScaleChannel', 'ShiftChannel', 'ShiftChannel2D', 'ControlChannel', 'ControlChannel2D', 'AlternateChannel', 'AlternateChannel2D', 'SelectChannel', 'SelectChannel2D', 'ScaleSpatial']:  # sum, concat
                ...
                yolo_out.append(module(x, out))  # x的shape为【batch_size,3*(5+num_classes), feature_w, feature_h】
            elif name == 'JDELayer':
